...
export default function GearError({
  ...
  return (
    <div className="flex flex-col items-start gap-4 py-16">
      <p className="text-xs uppercase tracking-widest text-zinc-500">Gear · Error</p>
      <h1 className="font-serif text-3xl text-zinc-50">Couldn&apos;t load this view</h1>
      <p className="max-w-prose text-zinc-400">
        {isDev
          ? (error.message ?? 'An unexpected error occurred.')
          : 'The gear catalogue is temporarily unavailable. The operator has been notified.'}
      </p>
      {error.digest && (
      ...
      <div className="flex gap-4">
        ...
        <Link href="/gear" className="text-sm text-zinc-500 hover:underline">
          Back to Gear
        </Link>
      </div>
    </div>
