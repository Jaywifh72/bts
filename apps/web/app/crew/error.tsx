...
export default function CrewError({
  ...
  return (
    <div className="flex flex-col items-start gap-4 py-16">
      <p className="text-xs uppercase tracking-widest text-zinc-500">Crew · Error</p>
      <h1 className="font-serif text-3xl text-zinc-50">Couldn&apos;t load this view</h1>
      <p className="max-w-prose text-zinc-400">
        {isDev
          ? (error.message ?? 'An unexpected error occurred.')
          : 'The crew index is temporarily unavailable. The operator has been notified.'}
      </p>
      {error.digest && (
      ...
      <div className="flex gap-4">
        ...
        <Link href="/crew" className="text-sm text-zinc-500 hover:underline">
          Back to Crew
        </Link>
      </div>
    </div>
